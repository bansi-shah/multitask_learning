...
def build_pspnet_multi_exp(inputs, label_size, num_classes, preset_model='PSPNetMultiExp', frontend="ResNet101", pooling_type = "MAX", weight_decay=1e-5, upscaling_method="conv", is_training=True, pretrained_dir="models"):
    """
    ...
    """
    Net 1 will do segmentation, Net 2 will do Regression, Net 3 will do FGBG
    """
    ...
    net3 = slim.conv2d(psp, 512, [3, 3], activation_fn=None)
    net3 = slim.batch_norm(net3, fused=True)
    net3 = tf.nn.relu(net3)

    if upscaling_method.lower() == "conv":
        net3 = ConvUpscaleBlock(net3, 256, kernel_size=[3, 3], scale=2)
        net3 = ConvBlock(net1, 256)
        net3 = ConvUpscaleBlock(net3, 128, kernel_size=[3, 3], scale=2)
        net3 = ConvBlock(net1, 128)
        net3 = ConvUpscaleBlock(net3, 64, kernel_size=[3, 3], scale=2)
        net3 = ConvBlock(net1, 64)
        
    elif upscaling_method.lower() == "bilinear":
        net1 = Upsampling(net3, label_size)
    
    net3 = slim.conv2d(net3, 2, [1, 1], activation_fn=None, scope='logits3')

    return net1, net2, net3, init_fn
