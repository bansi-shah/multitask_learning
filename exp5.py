...
loss3 = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits(logits = network3, labels = net_output3))

loss_param1 = tf.Variable(1.0, name='loss_param1')
loss_param2 = tf.Variable(1.0, name='loss_param2')
loss_param3 = tf.Variable(1.0, name='loss_param3')

final_loss = loss_param1*loss1 + loss_param2*loss2 + loss_param3*loss3
...
# opt1 = tf.train.RMSPropOptimizer(learning_rate=0.0001, decay=0.995).minimize(loss1, var_list=[var for var in tf.trainable_variables()])
# opt2 = tf.train.RMSPropOptimizer(learning_rate=0.0001, decay=0.995).minimize(loss2, var_list=[var for var in tf.trainable_variables()])
# opt3 = tf.train.RMSPropOptimizer(learning_rate=0.0001, decay=0.995).minimize(loss3, var_list=[var for var in tf.trainable_variables()])
...
for epoch in range(args.epoch_start_i, args.num_epochs):
    ...
    for i in range(num_iters):
        ...
        for j in range(args.batch_size):
            ...
            with tf.device('/cpu:0'):
                ...
                output3_image = np.float32(helpers.fgbg(label = output3_image, label_values = label_values, class_names = class_names_list))

                input_image_batch.append(np.expand_dims(input_image, axis=0))
                output1_image_batch.append(np.expand_dims(output1_image, axis=0))
                output2_image_batch.append(output2_image)
                output3_image_batch.append(np.expand_dims(output3_image, axis=0))

        if args.batch_size == 1:
            input_image_batch = input_image_batch[0]
            output1_image_batch = output1_image_batch[0]
            output2_image_batch = output2_image_batch[0]
            output3_image_batch = output3_image_batch[0]
        else:
            input_image_batch = np.squeeze(np.stack(input_image_batch, axis=1))
            output1_image_batch = np.squeeze(np.stack(output1_image_batch, axis=1))
            output2_image_batch = np.array(output2_image_batch)
            output3_image_batch = np.squeeze(np.stack(output3_image_batch, axis=1))

        # Do the training
        _, current1, current2, current3, = sess.run([opt, loss1, loss2, loss3], feed_dict = {net_input:input_image_batch, net_output1 : output1_image_batch, net_output2 : output2_image_batch, net_output3 : output3_image_batch})
        
        current_losses.append(current1+current2+current3)
        current_losses1.append(current1)
        current_losses2.append(current2)
        current_losses3.append(current3)

        cnt = cnt + args.batch_size
        if cnt % 20 == 0:
            string_print = "Epoch = %d Count = %d Current_Loss1 = %.4f Current_Loss2 = %.4f Current_Loss3 = %.4f Time = %.2f "%(epoch, cnt, current1, current2, current3, time.time()-st)
    ...
    mean_loss3 = np.mean(current_losses3)
    avg_loss3_per_epoch.append(mean_loss3)

    a1 = loss_param1.assign(mean_loss1/(mean_loss1+mean_loss2+mean_loss3))
    a2 = loss_param2.assign(mean_loss2/(mean_loss1+mean_loss2+mean_loss3))
    a3 = loss_param3.assign(mean_loss3/(mean_loss1+mean_loss2+mean_loss3))
    ...
    if epoch % args.validation_step == 0:
        ...
        for ind in val_indices:
            ...
            gt3 = utils.load_image(val_output1_names[ind])[:args.crop_height, :args.crop_width]
            ...
            output3_image = np.array(output3_image[0, :, :, :])
            output3_image = helpers.reverse_one_hot(output3_image)         
            ...
            # gt3 = helpers.reverse_one_hot(gt3)
            # gt3 = helpers.colour_code_fgbg(gt3)
        ...
        avg_scores_per_epoch_2.append(avg_score_2)
        ...
        avg_iou_per_epoch_2.append(avg_iou_2)
        ...
        print("\nAverage validation accuracy2 for epoch # %04d = %f"% (epoch, avg_score_2))
    ...
    fig2, ax2 = plt.subplots(figsize=(11, 8))
    ax2.plot(range(args.epoch_start_i, epoch+1), avg_loss3_per_epoch)
    ax2.set_title("Train Segmentation loss vs epochs")
    ax2.set_xlabel("Epoch")
    ax2.set_ylabel("Current loss")
    plt.savefig(args.checkpoint_path +'/train_segmentataion2_loss_vs_epochs.png')
    plt.clf()

    fig3, ax3 = plt.subplots(figsize=(11, 8))
    ax3.plot(list(range(args.epoch_start_i, epoch+1)), avg_iou_per_epoch_2)
    ax3.set_title("Validation average IoU vs epochs")
    ax3.set_xlabel("Epoch")
    ax3.set_ylabel("Current IoU")
